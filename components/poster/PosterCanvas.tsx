...
import { GenesisRarity, NFT_CLAIM } from '@/constants';
...
export default function PosterCanvas({ gamerInfo, gamerGames }: { gamerInfo?: GamerInfoData; gamerGames?: GamerGamesData }) {
  ...
  useEffect(() => {
    ...
    if (gamerInfo?.nft_claim !== NFT_CLAIM.CLAIMED || gamerInfo?.nft_level === GenesisRarity.Rekt) return;
    html2canvas(capture, {
      useCORS: true,
      allowTaint: true,
      scale: 1,
  ...
  if (gamerInfo?.nft_claim !== NFT_CLAIM.CLAIMED || gamerInfo?.nft_level === GenesisRarity.Rekt) return null;
